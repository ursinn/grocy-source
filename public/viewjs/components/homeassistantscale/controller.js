class HAScaleController {
	...
	setupEventHandlers() {
		...
		this._addEventHandler($(document), 'focus', 'input', (e) => {
			...
			if (this.model.config.scannerEnabled && this.model.config.scannerEntityId) {
				this.model.setScannerWaiting($input);
			}
		});
		...
		this._addEventHandler($(document), 'blur', 'input', (e) => {
			...
			if (this.model.scannerData.targetInput?.is(input)) {
				this.model.clearScannerWaiting($input);
			}
		});
		...
		this._addEventHandler($(document), 'keydown', (e) => {
			...
			} else if (e.key === 'w' && e.altKey && !e.ctrlKey && !e.shiftKey) {
				e.preventDefault();
	...
	_handleScannerHotkey() {
		HAScaleLogger.info('Controller', 'Scanner hotkey (Alt+W) triggered');
		...
		if (activeElement && activeElement.tagName === 'INPUT') {
			if (this.model.config.scannerEnabled && this.model.config.scannerEntityId) {
				if (this.model.scannerData.targetInput && this.model.scannerData.targetInput.is($activeElement)) {
					this.model.clearScannerWaiting($activeElement);
				} else {
					this.model.setScannerWaiting($activeElement);
				}
			} else {
				HAScaleUtils.showNotification('warning', 'Scanner not configured or disabled');
			}
			return;
		}

		this.model.config.scannerEnabled = !this.model.config.scannerEnabled;
		this.model.saveConfiguration();

		const statusMessage = this.model.config.scannerEnabled
			? HAScaleConstants.CONFIG.MESSAGES.SCANNER_ENABLED
			: HAScaleConstants.CONFIG.MESSAGES.SCANNER_DISABLED;

		HAScaleUtils.showNotification('info', statusMessage, {
			timeOut: HAScaleConstants.CONFIG.TIMEOUTS.NOTIFICATION_MEDIUM
		});

		const $checkbox = $('.scanner-enabled-checkbox');
		if ($checkbox.length > 0) {
			$checkbox.prop('checked', this.model.config.scannerEnabled);
		}

		if (!this.model.config.scannerEnabled) {
			this._clearAllScannerWaitingStyles();
		}
	}

	_clearAllScannerWaitingStyles() {
		const cssClass = HAScaleConstants.CONFIG.CSS_CLASSES.INPUT_SCANNER_WAITING;
		$(`.${cssClass}`).removeClass(cssClass);
	}

	_processScannerData(value) {
		if (!this.model.config.scannerEnabled) {
			return;
		}

		const $targetInput = this.model.scannerData.targetInput;
		if (!$targetInput || $targetInput.length === 0) {
			HAScaleLogger.warn('Controller', 'No target input found for scanner data');
			return;
		}

		HAScaleLogger.info('Controller', `Processing scanner data: ${value}`);
		...
		try {
			...
			setTimeout(() => {
				...
				$targetInput[0].dispatchEvent(tabEvent);
			}, 50);

			this.model.clearScannerWaiting($targetInput);

			HAScaleUtils.showNotification('success', `Scanner input: ${value}`);
		} catch (error) {
			HAScaleLogger.error('Controller', 'Error processing scanner data:', error);
			HAScaleUtils.showNotification('error', 'Error processing scanner data');
		}
	}
