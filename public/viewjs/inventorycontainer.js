...
function formatAmountWithUnit(amount) {
	...
	var unitText = __n(
		amount,
		CurrentStockEntry.product_details.quantity_unit_stock.name,
		CurrentStockEntry.product_details.quantity_unit_stock.name_plural,
		true,
	);
...
function isWeightChangeSignificant(currentWeight, expectedWeight) {
	return (
		Math.abs(currentWeight - expectedWeight) >= WEIGHT_PRECISION_TOLERANCE
	);
}
...
function checkWeightInconsistencyWarning(currentAmount, increaseAmount) {
	...
	if (percentageIncrease > 2) {
		var warningClass =
			percentageIncrease > 5 ? "text-danger" : "text-warning";
		var warningIcon =
		...
		return (
			'<br><div class="alert ' +
			(percentageIncrease > 10 ? "alert-danger" : "alert-warning") +
			' mt-2 mb-0">' +
			'<i class="fa-solid ' +
			warningIcon +
			'"></i> ' +
			"<strong>" +
			__t("Weight Inconsistency Warning") +
			":</strong> " +
			__t(
				"Adding %1$s (%2$s%% increase from current amount). Please verify this measurement is correct.",
...
function createScenarioAlert(type, iconClass, message) {
	...
	return (
		'<div id="inventory-scenario" class="alert ' +
		alertClass +
		' mt-2">' +
		'<i class="fa-solid ' +
		iconClass +
...
$("#container-inventory-form").on("submit", function (e) {
	...
	if (
		$("#source_stock_entry").hasClass("is-invalid") ||
		$("#destination_stock_entry").hasClass("is-invalid")
	) {
		return;
	}
	...
	$.ajax({
		...
		.done(function (response) {
			...
			if (response.success) {
				toastr.success(
					__t("Inventory saved successfully") +
						"<br><strong>" +
						__t("Net weight") +
						":</strong> " +
				...
				if (CurrentStockEntry && CurrentStockEntry.product) {
					Grocy.Components.ProductCard.Refresh(
						CurrentStockEntry.product.id,
					);
				}
...
function validateGrossWeight() {
	...
	if (grossWeight < CurrentContainerWeight) {
		setFieldValidation(
			...
			false,
			__t(
				"Gross weight cannot be less than container weight (%1$s)",
				formatAmountWithUnit(CurrentContainerWeight),
			),
		);
		return false;
	}
	...
	if (
		!isWeightChangeSignificant(grossWeight, ExpectedGrossWeight) &&
		CurrentStockEntry
	) {
		setFieldValidation(
			"gross_weight",
			false,
			__t(
				"No inventory change detected - adjust the gross weight to reflect actual inventory",
			),
		);
		return false;
	}
...
function validateStockSource() {
	...
	var partialTransferMode = $(
		'input[name="partial_transfer_mode"]:checked',
	).val();
	...
	if (!sourceStockEntry && !partialTransferMode) {
		setFieldValidation(
			...
			false,
			__t(
				"Please select where this stock came from or choose how to handle the stock increase",
			),
		);
		return false;
	}
...
function validateStockDestination() {
	...
	if (!destinationStockEntry) {
		setFieldValidation(
			...
			false,
			__t("Please select destination stock entry"),
		);
		return false;
	}
...
function parseGrocycode(barcode) {
	...
	if (!barcode.startsWith("grcy:p:")) {
		showError(
			__t(
				"Invalid barcode format. Expected container barcode (grcy:p:PRODUCT_ID:STOCK_ID)",
			),
		);
		return null;
	}
	...
	if (parts.length < 4) {
		showError(
			__t("Invalid barcode format. Missing stock entry information."),
		);
		return null;
	}
		...
		Grocy.Api.Get(
			...
			function (stockEntries) {
				...
				var locationId =
					...
					productDetails.product.location_id;
...
function getContainerWeight(stockId, callback) {
	...
	var apiCall = Grocy.Api.Get(
		"userfields/stock/" + stockId,
		function (userfieldValues) {
			// Get the container weight directly from the userfields object
			var containerWeightValue =
				userfieldValues.StockEntryContainerWeight;

			var weight = containerWeightValue
				? parseFloat(containerWeightValue) || 0
				: 0;
	...
	if (apiCall && typeof apiCall.fail === "function") {
		apiCall.fail(function () {
			...
			showError(
				__t(
					"Failed to load container weight information. Please try again.",
				),
			);
			Grocy.FrontendHelpers.EndUiBusy("container-details");
		});
...
function displayStockEntryDetails() {
	...
	$("#current_amount").text(
		formatAmountWithUnit(parseFloat(stockEntry.amount)),
	);
	...
	$("#best_before_date").text(
		moment(stockEntry.best_before_date).format("L"),
	);
	...
	// Use robust product ID lookup
	var productId = product.id || stockEntry.product_id;
	...
	consumeButton.attr('data-product-id', productId);
	...
	contextButton.attr('data-product-id', productId);
	...
	contextButton.attr('data-consume-url', U('/consume?embedded&product=' + productId + '&locationId=' + stockEntry.location_id + '&stockId=' + stockEntry.stock_id));
	contextButton.attr('data-transfer-url', U('/transfer?embedded&product=' + productId + '&locationId=' + stockEntry.location_id + '&stockId=' + stockEntry.stock_id));
	...
	var defaultGrossWeight =
		parseFloat(stockEntry.amount) + CurrentContainerWeight;
...
function updateInventoryScenario(netWeight) {
	...
	{
		scenarioHtml = createScenarioAlert(
			"increase",
			"fa-plus-circle",
			__t(
				"Stock increased by %1$s. Please specify where this stock came from.",
				formatAmountWithUnit(difference),
			),
		);
	...
	} else if (difference < 0) // Stock decreased
	{
		scenarioHtml = createScenarioAlert(
			...
			"fa-minus-circle",
			__t(
				"Stock decreased by %1$s. Please specify where this stock went.",
				formatAmountWithUnit(Math.abs(difference)),
			),
		);
		...
		Grocy.Api.Get(
			...
			function (stockEntries) {
				...
				var availableEntries = stockEntries.filter(function (entry) {
					return (
						entry.stock_id !==
						CurrentStockEntry.stock_entry.stock_id
					);
				});
				...
				availableEntries.forEach(function (entry) {
					...
					var unit =
						CurrentStockEntry.product_details.quantity_unit_stock
							.name;

					// Create detailed option text similar to consume form
					var optionText = __t(
						"Amount: %1$s %2$s; Due on %3$s; Bought on %4$s",
						amount,
						unit,
...
function showNoSourceOptions(context) {
	...
	var baseText = __t(
		"All %1$s will be added without a source transfer",
		formatAmountWithUnit(context.requiredIncrease),
	);
	var warningHtml = checkWeightInconsistencyWarning(
		context.currentAmount,
		context.requiredIncrease,
	);
...
function showPartialTransferOptions(context) {
	...
	$("#partial-transfer-message").text(
		__t(
			"Selected source only has %1$s available, but %2$s is needed",
	...
	updatePartialTransferDetails(
		context.availableFromSource,
...
function updatePartialTransferDetails(availableFromSource, remaining) {
	...
	if (selectedMode === "transfer_available_only") {
		var netWeightAfterTransfer =
			parseFloat(CurrentStockEntry.stock_entry.amount) +
			availableFromSource;
		$("#remaining-modification-details").text(
			__t(
				"Transfer only %1$s from source (final amount will be %2$s)",
		...
		var baseText = __t(
			"Transfer %1$s from source and add remaining %2$s without source",
			formatAmountWithUnit(availableFromSource),
...
function clearForm(keepBarcode = false) {
	...
	$(
		"#gross_weight, #net_weight, #source_stock_entry, #destination_stock_entry",
	).val("");
	...
	// Hide sections
	$(
		"#container-details, #stock-source-group, #stock-destination-group, #destination-stock-entry, #partial-transfer-info, #stock-modification-option, #container-actions",
	).addClass("d-none");
	$("#inventory-scenario").remove();
		...
		Grocy.Api.Get("objects/stock_log", function (stockLogEntries) {
			...
			var containerEntries = stockLogEntries
				.filter(function (entry) {
					return (
						entry.note && entry.note.includes("Container inventory")
					);
				})
				.sort(function (a, b) {
					return (
						new Date(b.row_created_timestamp) -
						new Date(a.row_created_timestamp)
					);
				})
			...
			if (containerEntries.length === 0) {
				tableHtml =
					'<p class="text-muted">' +
					__t("No recent container inventories found") +
					"</p>";
			} else {
				tableHtml =
				...
				containerEntries.forEach(function (entry) {
					var date = moment(entry.row_created_timestamp).format(
						"L LT",
					);
					...
					var typeClass =
						parseFloat(entry.amount) > 0
							...
							: "text-danger";
					var productName =
						productMap[entry.product_id] || __t("Unknown");
					var locationName =
						locationMap[entry.location_id] || __t("Unknown");
...
$(document).on("click", ".container-consume-button", function (e) {
	...
	var wasSpoiled = $(e.currentTarget).hasClass(
		"container-consume-button-spoiled",
	);
	...
	bootbox.dialog({
		message: __t(
			"Are you sure to consume %1$s %2$s of %3$s?",
			consumeAmount,
			productQuName,
			productName,
		),
				...
				callback: function () {
					...
					Grocy.Api.Post(
						...
						function (result) {
							...
							toastr.success(
								__t(
									"Removed %1$s %2$s of %3$s from stock",
									consumeAmount,
									productQuName,
									productName,
								) +
									'<br><a class="btn btn-secondary btn-sm mt-2" href="#" onclick="UndoStockBookingEntry(' +
									result[0].id +
									", " +
									stockRowId +
									", " +
...
function UndoStockBookingEntry(bookingId, stockRowId, productId) {
	Grocy.Api.Post(
		"stock/bookings/" + bookingId.toString() + "/undo",
		{},
		function (result) {
			ReloadContainerDetails();
...
$(window).on("message", function (e) {
	...
	if (
		...
		data.Payload.Message === "ProductChanged"
	) {
		var changedProductId = data.Payload.Payload;

		if (
			CurrentStockEntry &&
			CurrentStockEntry.product.id == changedProductId
		) {
			ReloadContainerDetails();
		}
	}
