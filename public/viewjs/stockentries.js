...
$(document).on("click", ".product-open-button", function (e) {
	...
	bootbox.dialog({
		message: __t(
			"Are you sure to mark %1$s %2$s of %3$s as opened?",
			openAmount,
			productQuName,
			productName,
		),
			...
			danger: {
				...
				callback: function () {
					...
					Grocy.Api.Post(
						...
						{ amount: openAmount, stock_entry_id: stockId },
						function (result) {
							RefreshStockEntryRow(stockRowId);
							Grocy.FrontendHelpers.EndUiBusy();
							toastr.success(
								__t(
									"Marked %1$s %2$s of %3$s as opened",
									openAmount,
									productQuName,
									productName,
								) +
...
if (Grocy.Components.ProductPicker) {
	Grocy.Components.ProductPicker.GetPicker().on("change", function (e) {
		...
		if (value == "all" || value == "" || value == null) {
			...
			stockEntriesTable
				.column(1)
				.search("xx" + NormalizeString(value) + "xx")
				.draw();
		}
	});
}
...
function RefreshStockEntryRow(stockRowId) {
	Grocy.Api.Get(
		"stock/entry/" + stockRowId.toString().split(":")[0],
		function (result) {
			var stockRow = $("#stock-" + stockRowId + "-row");
			var productId = stockRow
				.find(".stock-consume-button")
			...
			stockRow
				.find(".stock-consume-button")
				.attr("data-consume-amount", result.amount);
			stockRow
				.find(".product-open-button")
				.attr("data-open-amount", result.amount);
			stockRow
				.find(".stock-entry-context-menu-button")
				.attr("data-amount", result.amount);
			...
			if (result.open == 1) {
				...
				$(
					".product-open-button[data-stockrow-id='" +
						stockRowId +
						"']",
				).addClass("disabled");
			} else {
...
$(window).on("message", function (e) {
	...
	if (data.Message == "ProductChanged") {
		$(".stock-consume-button[data-product-id='" + data.Payload + "']").each(
			function () {
...
function UndoStockBookingEntry(bookingId, stockRowId, productId) {
	Grocy.Api.Post(
		...
		{},
		function (result) {
			Grocy.GetTopmostWindow().postMessage(
				WindowMessageBag(
					"BroadcastMessage",
					WindowMessageBag("ProductChanged", productId),
				),
				Grocy.BaseUrl,
			);
